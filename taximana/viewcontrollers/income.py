...
class Income(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.create_income(500*0.6, 500*0.2, bars)

        # Statistic 
        tk.Label(self, text="Income 50% of total expense for maintaince per month", bg='white', font=(FONT, 15)).place(x=320, y=660)
        tk.Label(self, text="Income 50% of total expense for salary per month", bg='white', font=(FONT, 15)).place(x=320, y=690)
        tk.Label(self, text="Average Income per month:", bg='white', font=(FONT, 15)).place(x=320, y=720)
    ...
    def create_income(self, e_height, c_height, bars):
        for bar in bars:
            bar.update()
            tk.Frame(bar, width=23, height=e_height, bg = '#2D319E').place(x=0, y=500-e_height)
            tk.Frame(bar, width=23, height=c_height, bg = '#B57F71').place(x=26, y=500-c_height)
